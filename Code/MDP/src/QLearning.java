...
{
	private double maxLearningRate, pjog,epsilon, maxepsilon;
		...
		if(reachedGoal(currState)) {
			...
			if(decayingLR){
			    learningRate = (1000.0*maxLearningRate)/(1000.0+numEpisodes);
			 	epsilon-= (maxepsilon-0.1)/4000;
			}
			else
			    learningRate = maxLearningRate;
			
			if(0==numEpisodes%50)
				System.out.println(numEpisodes+","+learningRate+","+evalPolicy()+","+epsilon);
